...
class VehicleHireConMasterFilterView(APIView):
    def post(self, request, *args, **kwargs):
        try:
            # Extract filters from the request body
            filters = request.data.get("filters", {})
            ...
            # Apply dynamic filters
            queryset = apply_filters(VehicalHireContract, filters)
...
class TripMemoPendingPaymentView(APIView):
    def post(self, request, *args, **kwargs):
        try:
            filters = request.data.get("filters", {})
            ...
            queryset = queryset.filter(
                ...
                flag=True,
                trip_memo_status='OK',
                balance_vehicle_hire__gt=0   # 🔥 pending payment
            )
            ...
            return Response({
                'status': 'success',
                'message': 'Pending TripMemo payment report',
                'data': serializer.data
            }, status=status.HTTP_200_OK)
