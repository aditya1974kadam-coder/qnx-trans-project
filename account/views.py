...
from .models import GSTMaster, ChequeCaseId,PartyBilling,VoucherReceiptType,VoucherReceiptBranch,MoneyReceipt,VoucherPaymentType,VoucherPaymentBranch,CashBook,BillingSubmission,DeductionReasonType,Deduction,Collection,LocalMemoDelivery,TripMemo
from .serializers import GSTMasterSerializer, ChequeCaseIdSerializer, DeductionLRetrieveSerializer,PartyBillingSerializer,VoucherReceiptTypeSerializer,VoucherReceiptBranchSerializer,MoneyReceiptSerializer,VoucherPaymentTypeSerializer,VoucherPaymentBranchSerializer,CashBookSerializer,BillingSubmissionSerializer,DeductionReasonTypeSerializer,DeductionSerializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if advance_amount < 0:
                return Response({
                    "status": "error",
                    "msg": "Advance amount cannot be negative."
                }, status=status.HTTP_400_BAD_REQUEST)
        except:
            return Response({
                "status": "error",
                "msg": "Invalid advance amount."
            }, status=status.HTTP_400_BAD_REQUEST)

        # ✅ TOTAL vs ADVANCE CHECK (IMPORTANT)
        total_amt = float(data.get('totla_amt', 0))
        if advance_amount > total_amt:
            return Response({
                "status": "error",
                "msg": "Advance amount cannot be greater than total amount."
            }, status=status.HTTP_400_BAD_REQUEST)
...
class CashBookRetrieveFilterView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # -------------------------------
            # ✅ ADVANCE CALCULATION START
            # -------------------------------

            branch_ids = filters.get("branch_name_id__in", [])

            # safe date convert
            date_obj = datetime.strptime(date, "%Y-%m-%d").date()

            # LCM
            lcm_advance = Collection.objects.filter(
                branch_name_id__in=branch_ids,
                date=date_obj,
                memo_status='OK'
            ).aggregate(total=Sum('advance'))['total'] or 0

            # LDM
            ldm_advance = LocalMemoDelivery.objects.filter(
                branch_name_id__in=branch_ids,
                date=date_obj,
                memo_status='OK'
            ).aggregate(total=Sum('advance'))['total'] or 0
            ...
            # LSH
            lsh_advance = TripMemo.objects.filter(
                branch_id__in=branch_ids,
                date=date_obj,
                trip_memo_status='OK'
            ).aggregate(total=Sum('less_advance'))['total'] or 0    
            ...
            if not cash_book:
                ...
                if not cash_book:
                    ...
                    return Response({
                        ...
                        'data': {
                            "opening_balance": cash_book.closing_balance,
                            "lcm_advance": lcm_advance,
                            "ldm_advance": ldm_advance,
                            "lsh_advance": lsh_advance
                        }
                    }, status=status.HTTP_200_OK)
...
class DeductionReasonMasterFilterView(APIView):
    def post(self, request, *args, **kwargs):
        try:
            # Extract filters from the request body
            filters = request.data.get("filters", {})
            ...
            # Apply dynamic filters
            queryset = apply_filters(DeductionReasonType, filters)
...
# CREATE BANK MASTER
class CreateBankMasterView(APIView):
    def post(self, request):
        serializer = BankMasterSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response({
...
# BANK MASTER LIST VIEW
class BankMasterListView(APIView):
    def post(self, request):
        search = request.data.get("search")
        banks = BankMaster.objects.all()
        if search:
            banks = banks.filter(
        ...
        serializer = BankMasterSerializer(banks, many=True)
        return Response({
...
# BANK MASTER DETAIL VIEW
class BankMasterDetailView(APIView):
    def post(self, request):
        bank_id = request.data.get("id")
        search = request.data.get("search", "").strip()
        # 1️⃣ If ID is provided
        if bank_id:
            try:
                bank = BankMaster.objects.get(id=bank_id)
            except BankMaster.DoesNotExist:
                return Response({
                    "message": "Bank not found"
                }, status=404)
        # 2️⃣ If search is provided
        elif search:
            parts = search.split()
            if len(parts) == 2:
                short_name, last_digits = parts
                query = Q(short_name__iexact=short_name) & Q(account_number__endswith=last_digits)
            else:
                value = parts[0]
                query = (
                    Q(short_name__iexact=value) |
                    Q(account_number=value) |
                    Q(account_number__endswith=value)
                )
            bank = BankMaster.objects.filter(query).first()
            if not bank:
                return Response({
                    "message": "Bank not found"
                }, status=404)
            ...
            return Response({
                "message": "Provide either 'id' or 'search'"
            }, status=400)
        serializer = BankMasterSerializer(bank)
        return Response({
...
# UPDATE BANK MASTER
class UpdateBankMasterView(APIView):
    def post(self, request):
        bank_id = request.data.get("id")
        if not bank_id:
            return Response({
        ...
        serializer = BankMasterSerializer(bank, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response({
...
# SOFT DELETE BANK
class SoftDeleteBankMasterView(APIView):
    def post(self, request):
        bank_id = request.data.get("id")
        account_number = request.data.get("account_number")
        if not bank_id and not account_number:
            return Response({
                "message": "Provide either 'id' or 'account_number'"
            }, status=status.HTTP_400_BAD_REQUEST)
        try:
            bank = BankMaster.objects.get(
                Q(id=bank_id) | Q(account_number=account_number)
            )
        except BankMaster.DoesNotExist:
            return Response({
                "message": "Bank not found"
...
# RESTORE BANK
class RestoreBankMasterView(APIView):
    def post(self, request):
        bank_id = request.data.get("id")
        account_number = request.data.get("account_number")
        if not bank_id and not account_number:
...
# HARD DELETE BANK
class HardDeleteBankMasterView(APIView):
    def post(self, request):
        bank_id = request.data.get("id")
        account_number = request.data.get("account_number")
        if not bank_id and not account_number:
        ...
        bank = BankMaster.objects.filter(
            Q(id=bank_id) | Q(account_number=account_number)
        ).first()
        if not bank:
            return Response({
...
class GenerateChequeCaseIdNumberView(APIView):
    """Generate the next case_id for a given branch (preview only, does not save)."""
    def post(self, request, *args, **kwargs):
        try:
            branch_id = request.data.get('branch_id')
            if not branch_id:
...
class CreateChequeCaseIdView(APIView):
    ...
    The case_id is auto-generated from the branch sequence."""
    def post(self, request, *args, **kwargs):
        try:
...
class ChequeCaseIdPendingView(APIView):
    """Retrieve all active PENDING ChequeCaseId records."""
    def post(self, request, *args, **kwargs):
...
class ChequeCaseIdFilterView(APIView):
    """Filter ChequeCaseId records by arbitrary fields."""
    def post(self, request, *args, **kwargs):
...
class UpdateChequeCaseIdView(APIView):
    """Partial update of an existing ChequeCaseId record."""
    def post(self, request, *args, **kwargs):
        record_id = request.data.get('id')
        if not record_id:
            return Response({'status': 'error', 'message': 'id is required.'},
...
class ResolveChequeCaseIdView(APIView):
    """Mark a PENDING ChequeCaseId as CLEARED."""
    def post(self, request, *args, **kwargs):
        record_id = request.data.get('id')
        if not record_id:
            return Response({'status': 'error', 'message': 'id is required.'},
...
class ChequeCaseIdSoftDeleteView(APIView):
    """Soft-delete a ChequeCaseId record (sets is_active=False, flag=False)."""
    def post(self, request, *args, **kwargs):
        record_id = request.data.get('id')
        if not record_id:
            return Response({'status': 'error', 'message': 'id is required.'},
