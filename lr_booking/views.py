...
class StandardRateFilterView(APIView):
    def post(self, request, *args, **kwargs):
        try:
            ...
            queryset = apply_filters(StandardRate, filters)
...
class LR_BokkingUpdateAPIView(APIView):    
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            is_used_in_booking_memo = BookingMemoLRs.objects.filter(
                lr_booking=instance
            ).exists()

            if is_used_in_booking_memo:
                return Response({
                "status": "error",
                "msg": "This LR is already used in a Booking Memo. You cannot modify it."
                }, status=status.HTTP_400_BAD_REQUEST)
            ...
            if serializer.is_valid():
                ...
                if serialized_table_data is not None:
                    ...
                    if not serialized_table_data:  # If no new descriptions are provided                      
                        ...
                        # descriptions = instance.descriptions.all()

                        # total_wt = sum([d.charged_weight or 0 for d in descriptions])
                        # total_pkg = sum([d.quantity or 0 for d in descriptions])

                        # instance.tchargedwt = total_wt
                        # instance.tpackage = total_pkg
                        # instance.save()
